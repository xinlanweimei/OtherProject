...
:init
call jtm\init.cmd
title SHOP++ JTM %version%


sc query |find /i "MsMysql">nul 2>nul
if not errorlevel 1 (
echo 初始化Mysql以下信息可以忽略 
sc stop MsMysql 
sc delete MsMysql 
) else (
echo off
...
sc query |find /i "MsTomcat">nul 2>nul	
if not errorlevel 1 (
echo 初始化Tomcat以下信息可以忽略
sc stop MsTomcat 
sc delete MsTomcat
) else (
echo off
...
echo SHOP++ JTM %version% 
echo http://www.shopxx.net
echo Tomcat Port: %tomcat_port%
echo MySQL  Port: %mysql_port%

echo -----------------------------------------------------------------
echo 1  -  启动Tomcat + MySQL                 2  -  停止Tomcat + MySQL
echo 3  -  启动Tomcat                         4  -  停止Tomcat
echo 5  -  启动MySQL                          6  -  停止MySQL
echo 7  -  设置Tomcat端口                     8  -  设置MySQL端口
echo 9  -  检查端口状态                       10 -  设置MySQL root密码
echo -----------------------------------------------------------------
echo 启动成功后窗口会自动关闭
echo -----------------------------------------------------------------
set /p input=请选择: 
...
:execute
if exist %1 (call %1 && goto :menu) else echo %1 不存在. & pause & exit
